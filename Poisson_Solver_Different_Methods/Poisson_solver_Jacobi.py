...
while(err1>5e-7):
    for i in range(1,nx-1):
        for j in range(1,ny-1):
            phi_new[i][j] = ( phi[i+1][j]+phi[i-1][j] + phi[i][j+1]+ phi[i][j-1]-rho[i][j])/4.0
    err = phi_new[:,:]-phi[:,:]
...
#out of the program------------------------------------------------------------------------------------------
#No. of iterations it takes to converge is 4780 for error 5e-7, computation time is around 17 sec
#--------------------------------------------------------------------------------------------------------------